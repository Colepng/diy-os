...
impl Device for IdeController {
    fn children(&self) -> Option<Box<dyn Iterator<Item = Arc<Mutex<dyn Device>>>>> {
        Some(Box::new(
            [
                self.drive_1.as_ref().cloned(),
                self.drive_2.as_ref().cloned(),
                self.drive_3.as_ref().cloned(),
                self.drive_4.as_ref().cloned(),
            ]
            .into_iter()
            .flatten()
...
fn init_channel(mut channel: Channel) -> (Option<Drive>, Option<Drive>) {
    ...
    let channel = Rc::new(Mutex::new(channel));

    (
        init_drive(channel.clone(), DriveType::Parent),
        init_drive(channel, DriveType::Child),
    )
}

fn init_drive(channel_lock: Rc<Mutex<Channel>>, drive_type: DriveType) -> Option<Drive> {
...
impl Channel {
    ...
    fn write_hdd_sel(&mut self, value: HddSelect) {
